...
public class JDBCBroker {
	...
	protected JDBCBroker() {
		...
		try {
			...
			new Event(Event.getLeafLevelClassName(this), "JDBCBroker",
					"Could not load driver class[" + driverClassName + "]", Event.ERROR);
		...
		if (myInstance != null) {
			if (theDBConnection == null) {
				if ((dbName != null) & (username != null) && (password != null)) {
					try {
						// Create a connection to the database
						theDBConnection = theDriver
								.connect("jdbc:" + dbInstallationNameVal + "://" + server + ":3306/" +
										dbName + "?" + "user=" + username + "&password=" +
										password, null);
					} catch (SQLException exc) {
						System.err.println(
								"JDBCBroker.getConnection - Could not connect to database!" + "\n" + exc.getMessage());
						// new Event(Event.getLeafLevelClassName(this), "getConnection", "Could not
						// connect to database", Event.ERROR);
					}
				}
			}
		}
		// System.out.println("JDBCBroker.getConnection() with connection " +
		// theDBConnection);
	...
	protected void finalize() {
		if (theDBConnection != null) {
			try {
				...
				new Event(Event.getLeafLevelClassName(this), "releaseConnection", "Could not release connection",
						Event.WARNING);
			}
		}
	}
