...
public class Patron extends EntityBase implements IView
{
	...
	public Patron(String patronId) 
		throws InvalidPrimaryKeyException 
		{
			...
			Vector<Properties> allDataRetrieved = getSelectQueryResult(query); // get attributes from patron table as "properties"

			// You must get one account at least ie if we received some data from patron table
			if (allDataRetrieved != null)
			{
				...
				else
				{
					...
					// populating persistentState with data from patron table aka constructing Patron from that data
	...
	public Patron(Properties props)
	{
		...
		while (allKeys.hasMoreElements() == true)
		{
			...
			if (nextValue != NULL)
			{
				...
				persistentState.setPropery(nextKey, nextValue);
			}
		}
	...
	private void updateStateInDatabase()
	{
		try
		{
			if (persistentState.getProperty("patronId") != NULL) // we are updating an existing patrons data
			{
				...
				whereClause.setProperty("patronId"); // specify which field for query
				persistentState.getProperty("patronId"); // get data to save from persistent state
				updatePersistentState(mySchema, persistentState, whereClause); 
				updateStatusMessage("Patron data for patronId: ", persistentState.getProperty("patronId"), " successfully updated in database!");
			}
			else // "saving" new data, inserting new row to table
			{
				Integer patronId = insertAutoIncrementalPersistentState(mySchema, persistentSTate); // patronId is auto-generated by dbms
				persistentState.setProperty("patronId", "" + patronId.intValue());
				updateStatusMessage("Patron data for new patronId: ", persistentState.getProperty("patronId"), " successfully saved in database!");
			}
		}
